...
const Success = () => {
    ...
    useEffect(() => {
        if (!sessionId) {
            navigate('/');
            return; // Exit early if sessionId is not present
        }

        if (cartProduct.length === 0) {
            return; // Exit early if there are no cart products
        }
        ...
        saveOrder();
    }, [sessionId, navigate, currentUser?.email, cartProduct, resetCart, currentUser?.id]);
            ...
            <div className="min-h-[400px] flex flex-col items-center justify-center gap-y-5">
                <h2 className="text-2xl md:text-4xl font-bold text-center">
                    {loading
                        ...
                        : "Your Payment Accepted by E-Commerce.com"}
                </h2>
                <p>
