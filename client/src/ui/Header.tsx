...
import { TbBellHeart, TbUserSquare } from "react-icons/tb";
...
const Header = () => {
    ...
    return (
        <div className="w-full bg-whiteText md:sticky md:top-0 z-50">
            <div className="max-w-screen-xl mx-auto h-20 flex items-center justify-between px-4 lg:px-0">
                ...
                <div className="hidden md:inline-flex max-w-3xl w-full relative">
                    <input
                        ...
                        focus:right-1 focus:ring-darkText sm:text-sm px-4 py-2
                        "
                    />
                    {searchText ? (
                        <IoClose
                ...
                <div className="flex items-center gap-x-6 text-2xl">
                    <Link to={"/profile"}>
                        {currentUser ? (
                            ...
                            <TbUserSquare className="hover:text-skyText duration-200 cursor-pointer" />
                        )}
                    </Link>
                    <Link to={"/favorite"} className="relative block">
                        <TbBellHeart className="hover:text-skyText duration-200 cursor-pointer" />
                    ...
                    <Link to={"/cart"} className="relative block">
                        <BsBagCheck className="hover:text-skyText duration-200 cursor-pointer" />
                        <span className="inline-flex items-center justify-center bg-redText text-whiteText absolute -top-1 -right-2 text-[9px] rounded-full w-4 h-4">
            ...
            <div className="w-full bg-blue-500 text-whiteText">
                <Container className="py-2 max-w-4xl flex items-center gap-5 justify-between">
                    <Menu>
                        ...
                        >
                            <MenuItems
                                anchor="bottom end"
                                className="w-52 origin-top-right rounded-xl border border-white/5 bg-green-500 p-1 text-sm/6 text-gray-300 [--anchor-gap:var(--spacing-1)] focus:outline-none hover:text-white z-50"
                            >
                                {categories.map((item) => (
